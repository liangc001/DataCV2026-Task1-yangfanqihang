"""
VQA Challenge solver interface and batch runner.
...
def run(solver: Solver, input_csv: str, output_txt: str, output_json: str = "model.json"):
    ...
    if pd is not None:
        ...
        with open(input_csv, "r", encoding="utf-8", newline="") as f:
            ...
            assert "image_path" in cols and "prompt" in cols
            rows = list(rd)
        for idx, row in tqdm(enumerate(rows), total=len(rows), desc="Processing"):
            results[idx] = solver.solve(str(row.get("image_path", "")), str(row.get("prompt", "")))
